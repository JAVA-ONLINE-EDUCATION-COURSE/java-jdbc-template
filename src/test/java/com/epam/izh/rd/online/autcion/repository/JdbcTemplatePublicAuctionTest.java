...
class JdbcTemplatePublicAuctionTest {
  ...
  @Autowired private PublicAuction publicAuction;
  @Autowired private JdbcTemplate jdbcTemplate;
  @Autowired private RowMapper<Bid> bidRowMapper;
  @Autowired private RowMapper<Item> itemRowMapper;
  @Autowired private RowMapper<User> userRowMapper;
  ...
  private void setup() {
    ...
    user3 = new User(3L, "address3", "VASYA3", "VASYALogin3", "VASYAPass3");
  ...
  void getAvgItemCost() {
    ...
    userAndAvgItemCost.put(user1, 75.0);
    userAndAvgItemCost.put(user2, 120.0);
  ...
  void getMaxBidsForEveryItem() {
    ...
    itemsAndBids.put(item1, bid2);
    itemsAndBids.put(item2, bid3);
  ...
  void getUserActualBids() {
    List<Bid> userActualBids = publicAuction.getUserActualBids(user2.getUserId());

    assertIterableEquals(singletonList(bid2), userActualBids);
  }
  ...
  void createUser() {
    ...
    User dbItem =
        jdbcTemplate.queryForObject("SELECT * FROM users WHERE user_id=?", userRowMapper, 4);
  ...
  void createItem() {
    ...
    Item dbItem =
        jdbcTemplate.queryForObject("SELECT * FROM items WHERE item_id=?", itemRowMapper, 4);
  ...
  private LocalDate transformDate(String string) {
    return LocalDate.parse(string, DT_FORMATTER);
  }
}
